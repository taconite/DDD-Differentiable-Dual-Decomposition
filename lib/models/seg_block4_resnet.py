...
class SegResNet(nn.Module):
    ...
    def init_weights(self, init_deconvs, pretrained=''):
        if os.path.isfile(pretrained):
            for name, m in self.conv_bottleneck.named_modules():
                if isinstance(m, nn.BatchNorm2d):
                    logger.info('=> init conv_bottleneck.{}.weight as 1'.format(name))
                    logger.info('=> init conv_bottleneck.{}.bias as 0'.format(name))
                    nn.init.constant_(m.weight, 1)
                    nn.init.constant_(m.bias, 0)

            # Initialize feature classification layers with zero-mean gaussian weights and 0 bias,
            # contrary to default pytorch initialization which assumes ensuing ReLU activation
            for name, m in self.feature_layer.named_modules():
                if isinstance(m, nn.BatchNorm2d):
                    logger.info('=> init feature_layer.{}.weight as 1'.format(name))
                    logger.info('=> init feature_layer.{}.bias as 0'.format(name))
                    nn.init.constant_(m.weight, 1)
                    nn.init.constant_(m.bias, 0)
